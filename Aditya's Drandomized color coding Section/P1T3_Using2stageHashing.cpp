...
// ------------------------- Polynomial Hash -------------------------
// Implements a polynomial hash function over GF(p)
class PolynomialHash
{
private:
    vector<int> coefficients; // coefficients: a0, a1, ..., a_(k-1)
    int prime;                // modulus for the field
...
public:
    ...
    PolynomialHash(int k, int p) : prime(p)
    {
        ...
        // Make sure the polynomial is not constant (for k>1)
        ...
        // cout << "[PolynomialHash::PolynomialHash] Time taken: " << duration.count() << " ns" << endl;
    }
    ...
    int evaluate(int x) const
    {
        ...
        // auto end = high_resolution_clock::now();
        // auto duration = duration_cast<nanoseconds>(end - start);
        // cout << "[PolynomialHash::evaluate] Time taken: " << duration.count() << " ns" << endl;
...
// Constructs a family of polynomial-based hash functions.
class KPerfectHashFamily
{
private:
    int k;                                // intended range size (used to set degree)
    int n;                                // universe size (e.g., maximum vertex id)
    int prime;                            // prime modulus for the field
    vector<PolynomialHash> hashFunctions; // Collection of hash functions
    ...
    bool isPrime(int num) const
    {
        ...
        if (num <= 1)
        {
            // auto end = high_resolution_clock::now();
            // auto duration = duration_cast<nanoseconds>(end - start);
            // cout << "[KPerfectHashFamily::isPrime] Time taken: " << duration.count() << " ns" << endl;
            return false;
        }
    ...
    // Utility: Find next prime number greater than "start".
...
public:
    // Constructor: "numFunctions" determines the size of the family.
    KPerfectHashFamily(int k_val, int n_val, int numFunctions) : k(k_val), n(n_val)
    {

        // Choose a prime larger than max(n, k)
        prime = nextPrime(max(n, k));
    ...
    // Returns a hash function (a PolynomialHash) from the family.
...
private:
    ...
    KPerfectHashFamily stage1;
    // Stage2: maps from [0, k^2 - 1] to [0, k - 1]
    KPerfectHashFamily stage2;
...
public:
    ...
    TwoStageKPerfectHash(int k_val, int n_val)
        : k(k_val),
          stage1(k_val, n_val, 2 * k_val * static_cast<int>(ceil(log2(n_val)))),
          stage2(k_val, k_val * k_val, k_val * k_val)
    {
    }
