...
{
private:
    unique_ptr<LSFRHashFamily> stage1;
    unique_ptr<LSFRHashFamily> stage2;
    int k;

public:
    TwoStageKPerfectHash(int k_val, int n_val)
        : k(k_val),
          stage1(make_unique<LSFRHashFamily>(
              k_val, n_val, 2 * k_val * static_cast<int>(ceil(log2(n_val))))),
          stage2(make_unique<LSFRHashFamily>(
              k_val, k_val * k_val, k_val * k_val)) {}
